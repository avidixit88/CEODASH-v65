...
MAX_COMPANY_SITE_CANDIDATE_URLS = 10
MAX_COMPANY_SITE_LINKS_PER_PAGE = 16
...
def _expand_validated_company_urls(source: SponsorEvidenceSource, seed_url: str) -> list[tuple[int, str]]:
    ...
    for sub in COMMON_COMPANY_EVIDENCE_SUBDOMAINS:
        for path in ("/news-releases", "/news", "/press-releases", "/events-presentations", "/events", "/publications"):
    ...
    for url in dict.fromkeys(candidates):
        ...
        if len(out) >= max(MAX_COMPANY_SITE_CANDIDATE_URLS * 2, 12):
            break
